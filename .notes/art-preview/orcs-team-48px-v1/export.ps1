...
public static class SpriteExport {
  ...
  public static void Preview(string[] paths,string[] labels,string target) {
    ...
    using(var border=new Pen(Color.FromArgb(117,129,91))) {
      ...
      g.DrawString("ORCS / SAMPLE TEAM 01",title,ink,24,16);
      g.DrawString("16 unique players - 48px sprites - 4x preview - charcoal / burnt orange",sub,ink,26,58);
      for(int i=0;i<paths.Length;i++) {
        int x=24+(i%4)*244, y=106+(i/4)*254;
        g.FillRectangle(tile,x,y+30,192,192); g.DrawRectangle(border,x,y+30,192,192);
        using(var image=new Bitmap(paths[i])) {
          g.InterpolationMode=InterpolationMode.NearestNeighbor;
          g.PixelOffsetMode=PixelOffsetMode.Half;
          int w=image.Width*4,h=image.Height*4;
          g.DrawImage(image,new Rectangle(x+(192-w)/2,y+30+192-h,w,h),0,0,image.Width,image.Height,GraphicsUnit.Pixel);
        }
...
$originalDir = Join-Path $assetRoot '../orcs-team-v1/originals'
...
foreach ($entry in $sourceEntries) {
  ...
  $canvas = 48
  $maxSize = 46
  if ($entry.id -like '*troll*') { $canvas = 60; $maxSize = 58 }
  if ($entry.id -like '*goblin*') { $maxSize = 33 }
  ...
  $check.Dispose()
  $manifest += [pscustomobject]@{id=$entry.id; file=('sprites/'+$entry.id+'.png'); width=$canvas; height=$canvas; tileWidth=48; tileHeight=48; anchor='bottom-center'; bounds=@{x=$bounds.X;y=$bounds.Y;width=$bounds.Width;height=$bounds.Height}}
  $paths += $target
