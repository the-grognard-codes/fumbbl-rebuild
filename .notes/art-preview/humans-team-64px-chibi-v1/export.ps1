...
public static class SpriteExport {
  ...
  public static void Preview(string[] paths,string[] labels,string target) {
    ...
    using(var border=new Pen(Color.FromArgb(117,129,91))) {
      ...
      g.DrawString("16 unique players - 64px chibi - 3x preview - blue / ivory",sub,ink,26,58);
...
foreach ($entry in $sourceEntries) {
  ...
  $canvas = 64
  $maxSize = 60
  if ($entry.id -like '*ogre*') { $canvas = 80; $maxSize = 76 }
  if ($entry.id -like '*halfling*') { $maxSize = 46 }
