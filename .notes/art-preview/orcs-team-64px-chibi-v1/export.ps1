...
public static class SpriteExport {
  ...
  public static void Preview(string[] paths,string[] labels,string target) {
    ...
    using(var border=new Pen(Color.FromArgb(117,129,91))) {
      ...
      g.DrawString("16 unique players - 64px chibi - 3x preview - charcoal / burnt orange",sub,ink,26,58);
      for(int i=0;i<paths.Length;i++) {
        ...
        using(var image=new Bitmap(paths[i])) {
          g.InterpolationMode=InterpolationMode.NearestNeighbor;
          g.PixelOffsetMode=PixelOffsetMode.Half;
          int w=image.Width*3,h=image.Height*3;
...
foreach ($entry in $sourceEntries) {
  ...
  $canvas = 64
  $maxSize = 60
  if ($entry.id -like '*troll*') { $canvas = 80; $maxSize = 76 }
  if ($entry.id -like '*goblin*') { $maxSize = 46 }
  ...
  $check.Dispose()
  $manifest += [pscustomobject]@{id=$entry.id; file=('sprites/'+$entry.id+'.png'); width=$canvas; height=$canvas; tileWidth=64; tileHeight=64; anchor='bottom-center'; bounds=@{x=$bounds.X;y=$bounds.Y;width=$bounds.Width;height=$bounds.Height}}
  $paths += $target
