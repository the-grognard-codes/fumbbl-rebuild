...
public static class SpriteExport {
  ...
  public static void Preview(string[] paths,string[] labels,string target) {
    ...
    using(var border=new Pen(Color.FromArgb(117,129,91))) {
      ...
      g.DrawString("16 unique players - 48px sprites - 4x preview - blue / ivory",sub,ink,26,58);
...
$originalDir = Join-Path $assetRoot '../humans-team-v1/originals'
...
foreach ($entry in $sourceEntries) {
  ...
  $canvas = 48
  $maxSize = 46
  if ($entry.id -like '*ogre*') { $canvas = 60; $maxSize = 58 }
  if ($entry.id -like '*halfling*') { $maxSize = 33 }
