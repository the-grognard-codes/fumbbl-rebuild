...
| M5: desktop match GUI and MVP integration | Decide DOM/SVG versus Pixi board through parity evidence under ADR-002; bring the approved pitch MVP into the authoritative `/browser/v2` match, with separate setup and live-match surfaces, React/DOM controls and reviewed artwork; depends on M3 match contract and M4 service safeguards | Two authorized players and a spectator use the polished GUI through supported setup, actions, prompts, reconnect/retry and full time; no mock outcomes or second authority; visual/interaction parity, keyboard and screen-reader paths, browser/zoom matrix, renderer failure path and shipped-asset disposition recorded | 20–40 days, provisional pending renderer spike |
...
The [Orc content readiness record](m6-orc-content-readiness.md) is an independently testable M6 content slice while M5 visual comparison continues. It records source facts and engine mapping evidence; match-ready Orc support still requires the listed catalog, saved-team, freeze and gameplay gates.
...
M5 begins with a focused parity spike: render the same crowded 26×15 fixture, 64:56 sprites, risk overlays, path, selection and zoom/Fit behavior in both the current DOM/SVG preview and a Pixi board. Compare appearance at 1280×720 and 1920×1080, 100%/200% zoom, input/focus behavior, asset/renderer failure, memory and frame timing. Record an ADR-002 addendum before adopting the board renderer. Keep scoreboard, dugouts, action controls, log/chat and accessible state/prompt/error summaries in React DOM. Port the board behind a presentation-only interface, then replace preview simulation data with server snapshots and server-issued legal actions; the same validated command path serves the visible Commit control and Space. M5 repeats applicable M4 R6 browser, accessibility and provenance checks against the integrated release candidate; M1 diagnostic Pixi evidence and local preview screenshots do not satisfy that gate. The [M5 renderer parity plan](m5-renderer-parity-plan.md) records the owner-reviewed scope and decision gates for the first slice.
