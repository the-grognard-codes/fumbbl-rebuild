...
## Version 4 adjustment

User request: “As the name is moles under the pitch, the mole character should be below the pitch.” Version 4 edits the existing vector composition: move the pitch overhead, place the unchanged mole below it, and add a restrained stepped soil tunnel cross-section. Apply the same arrangement to the email illustration and optical favicon. Preserve all wordmark contours and export sizes and regenerate the preview sheet. No new image-generation call was needed for this vector-layout edit; the original generation prompts below document the retained character and typography studies.
