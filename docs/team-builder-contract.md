...
1. A player signs in before using `/teambuilder`. The builder loads the versioned team catalog from the server. The first match-ready catalog is the existing BB2025 Human exhibition preset with a 1,150,000-gold budget. The page offers only content that the server can accept for a match.
...
- The old browser draft and JSON export format require no compatibility path. Account save/load is the first product contract; JSON import/export is deferred. Orcs, freeform rosters, progression and any additional catalog become match-ready only after the server supports and versions them.

## Contract checks for implementation

- A signed-in player can build, validate, save, reload and select a Human team by its name. The Play list shows only that account's teams and does not enable retired catalogs.
