...
test('SpectatorClient authenticates before browsing and sends only the selected match identifier', async () => {
  globalThis.WebSocket = MockSocket;
  const { SpectatorClient } = await loadClient();
  const socket = new MockSocket(); const statuses = []; const browse = [];
  const client = new SpectatorClient({ url: 'wss://game.example/browser/v2', getToken: async () => 'fresh-token', makeSocket: () => socket, onStatus: value => statuses.push(value), onBrowse: items => browse.push(items), onState: () => {} });
  client.connect(); socket.open(); await new Promise(resolve => setImmediate(resolve));
  assert.deepEqual(socket.sent[0], { version: 2, type: 'authenticate', requestId: socket.sent[0].requestId, bearer: 'fresh-token' });
  socket.message({ version: 2, type: 'authentication', requestId: 'auth', code: 'ACCEPTED' });
  assert.equal(socket.sent[1].type, 'spectatorBrowse');
  socket.message({ version: 2, type: 'spectatorBrowse', requestId: 'list', code: 'ACCEPTED', matches: [{ matchId: '12345678-1234-1234-1234-123456789abc', label: 'Home vs Away' }] });
  assert.equal(browse[0][0].label, 'Home vs Away');
  client.watch('12345678-1234-1234-1234-123456789abc');
  assert.deepEqual(socket.sent.at(-1).matchId, '12345678-1234-1234-1234-123456789abc');
  assert.equal(statuses.includes('Choose a game to watch.'), true);
});
