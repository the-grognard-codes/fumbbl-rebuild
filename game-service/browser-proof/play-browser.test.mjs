// Historical proof driver; the active site now uses the unified React runtime.
...
const socketMock = () => {
  class WebSocket {
    ...
    send(raw) {
      ...
      if (message.type === 'reissue') setTimeout(() => this.onmessage?.({ data: JSON.stringify({ type: 'session', code: 'fedcba9876543210fedcba9876543210', selfSlot: 0, slots: [{ occupied: true, connected: true }, { occupied: false, connected: false }], events: [] }) }), 0);
      if (message.type === 'releaseOpponent') setTimeout(() => this.onmessage?.({ data: JSON.stringify({ type: 'session', code: '00112233445566778899aabbccddeeff', selfSlot: 0, slots: [{ occupied: true, connected: true }, { occupied: false, connected: false }], events: [] }) }), 0);
...
test('mocked Firebase play flow renders a session in two browser contexts', { timeout: 15000 }, async () => {
  ...
  try {
    ...
    await assert.doesNotReject(creator.getByRole('button', { name: 'Refresh invite' }).waitFor());
    await creator.getByRole('button', { name: 'Refresh invite' }).click();
    await creator.waitForURL(`${site.url}/play#fedcba9876543210fedcba9876543210`);
    await creator.evaluate(() => window.__lastSocket.onmessage({ data: JSON.stringify({ type: 'session', code: 'fedcba9876543210fedcba9876543210', selfSlot: 0, slots: [{ occupied: true, connected: true }, { occupied: true, connected: false }], events: [] }) }));
    await assert.doesNotReject(creator.getByRole('button', { name: 'Release disconnected opponent' }).waitFor());
    await creator.getByRole('button', { name: 'Release disconnected opponent' }).click();
    await creator.waitForURL(`${site.url}/play#00112233445566778899aabbccddeeff`);
