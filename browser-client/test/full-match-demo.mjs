...
const draft = (teamIndex) => ({ catalogVersion: 'bb2025-exhibition-2026-09-25.1', ruleset: 'BB2025', rosterId: 'human', presetId: 'exhibition-1150', captainId: null, players: Array(11).fill('lineman').map((positionId, index) => ({ id: `p${index + 1}`, slot: index + 1, positionId: process.env.M4_PROCESS_KILL === '1' && teamIndex === 1 && index === 0 ? 'ogre' : positionId, skillIds: [] })), resources: { rerolls: process.env.M4_PROCESS_KILL === '1' && teamIndex === 0 ? 0 : 4, assistantCoaches: 0, cheerleaders: 0, apothecary: 1, dedicatedFans: 0 } });
