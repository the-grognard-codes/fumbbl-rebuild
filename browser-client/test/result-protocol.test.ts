...
test('decodes results from all frozen Human catalog versions', () => {
  const previous = 'bb2025-human-2026-09-24.1';
  const current = 'bb2025-human-2026-09-25.1';
  assert.equal(decodeMatchResult(JSON.stringify({ ...response, result: { ...result, catalogVersion: previous, presetVersion: previous } })).result?.catalogVersion, previous);
