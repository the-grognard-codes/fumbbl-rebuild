...
export function decodeSetupStateValue(value: unknown, spectator = false): SetupState {
	const result = object(value, ['matchId','revision','callerRole','phase','actor','prompt','players','weather','homeRerolls','awayRerolls','actions','turn','turnMode','ball','activePlayerId','half','homeTurn','awayTurn','homeScore','awayScore','drive']); text(result.matchId, 36); if (!uuid.test(result.matchId)) throw Error('Invalid match'); integer(result.revision, 2147483646); if (!(spectator && result.callerRole === 'spectator')) role(result.callerRole); role(result.actor);
