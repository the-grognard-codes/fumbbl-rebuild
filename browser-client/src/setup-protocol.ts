...
export function decodeSetupStateValue(value: unknown, spectator = false): SetupState {
	...
	if (!(keys.length === base.length || keys.length === base.length + 1 && Object.hasOwn(source, 'saveResume')) || base.some(key => !Object.hasOwn(source, key))) throw Error('Unexpected fields');
