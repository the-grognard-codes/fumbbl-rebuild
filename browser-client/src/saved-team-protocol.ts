...
export type SavedDocument = { formatVersion: 1 | 2 | 3; teamId: string; documentVersion: number; ruleset: 'BB2025'; catalogVersion: string; owner: { namespace: 'local' | 'account'; subject: string }; draft: TeamDraft; validation: SavedValidation };
export type SavedTeamSummary = { teamId: string; documentVersion: number; catalogVersion: string; teamName?: string; rosterId?: string; eligibility?: string };
export type SavedTeamResponse = { version: 1; type: 'savedTeam'; requestId: string; code: SavedTeamCode; document: SavedDocument | null; versionStatus: null | 'CURRENT' | 'MIGRATION_REQUIRED' | 'VERSION_UNAVAILABLE'; validation: SavedValidation | null; teams: SavedTeamSummary[] };
export type SavedTeamCode = 'OK' | 'MALFORMED_TEAM_REQUEST' | 'INVALID_DOCUMENT_VERSION' | 'CONFLICT' | 'NOT_FOUND' | 'VALIDATION_FAILED' | 'MIGRATION_REQUIRED' | 'VERSION_UNAVAILABLE' | 'PERSISTENCE_FAILED' | 'SAVE_OUTCOME_UNKNOWN' | 'DELETE_OUTCOME_UNKNOWN' | 'LIMIT_REACHED';
export type SavedTeamRequest =
  ...
  | { version: 1; type: 'savedTeam'; requestId: string; operation: 'delete'; teamId: string; expectedDocumentVersion: number }
  | { version: 1; type: 'savedTeam'; requestId: string; operation: 'import'; document: SavedDocument };

const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
const codes = new Set<SavedTeamCode>(['OK', 'MALFORMED_TEAM_REQUEST', 'INVALID_DOCUMENT_VERSION', 'CONFLICT', 'NOT_FOUND', 'VALIDATION_FAILED', 'MIGRATION_REQUIRED', 'VERSION_UNAVAILABLE', 'PERSISTENCE_FAILED', 'SAVE_OUTCOME_UNKNOWN', 'DELETE_OUTCOME_UNKNOWN', 'LIMIT_REACHED']);
...
function name(value: unknown, max: number): asserts value is string { if (typeof value !== 'string' || value.length === 0 || [...value].length > max || value !== value.trim().normalize('NFC') || /[\u0000-\u001f\u007f-\u009f]/.test(value)) throw Error('Invalid name'); }
...
function validateDraft(value: unknown): TeamDraft {
  const named = !!value && typeof value === 'object' && Object.hasOwn(value, 'draftVersion');
  const draft = object(value, [...(named ? ['draftVersion', 'teamName'] : []), 'catalogVersion', 'ruleset', 'rosterId', 'presetId', 'captainId', 'players', 'resources']);
  if (named) { if (draft.draftVersion !== 2) throw Error('Unsupported draft'); name(draft.teamName, 50); }
  ...
  const players = array(draft.players, 16); const ids = new Set<string>(); const slots = new Set<number>(); const jerseys = new Set<number>();
  for (const item of players) { const player = object(item, ['id', 'slot', ...(named ? ['jerseyNumber', 'playerName'] : []), 'positionId', 'skillIds']); text(player.id, 80); text(player.positionId, 80); integer(player.slot, 16); if (named) { integer(player.jerseyNumber, 99); name(player.playerName, 30); if (player.jerseyNumber < 1 || jerseys.has(player.jerseyNumber as number)) throw Error('Duplicate or invalid jersey'); jerseys.add(player.jerseyNumber as number); } if (player.slot < 1 || ids.has(player.id) || slots.has(player.slot)) throw Error('Duplicate player'); ids.add(player.id); slots.add(player.slot); for (const skill of array(player.skillIds, 2)) text(skill, 80); }
...
export function decodeSavedDocument(value: unknown): SavedDocument {
  ...
  if (![1, 2, 3].includes(document.formatVersion as number) || document.ruleset !== 'BB2025') throw Error('Unsupported document'); text(document.teamId, 36); if (!uuid.test(document.teamId)) throw Error('Invalid team id'); integer(document.documentVersion); if (document.documentVersion < 1) throw Error('Invalid document version'); text(document.catalogVersion, 100);
  const owner = object(document.owner, ['namespace', 'subject']); if (document.formatVersion === 1 ? owner.namespace !== 'local' || (owner.subject !== 'home' && owner.subject !== 'away') : owner.namespace !== 'account' || typeof owner.subject !== 'string' || !uuid.test(owner.subject)) throw Error('Invalid owner');
  const draft = validateDraft(document.draft); if ((document.formatVersion === 3) !== (draft.draftVersion === 2) || draft.ruleset !== document.ruleset || draft.catalogVersion !== document.catalogVersion) throw Error('Document/draft mismatch');
...
export function decodeSavedTeam(textJson: string): SavedTeamResponse {
  ...
  const teams = array(response.teams, 50); for (const team of teams) { const named = !!team && typeof team === 'object' && Object.hasOwn(team, 'eligibility'); const entry = object(team, ['teamId', 'documentVersion', 'catalogVersion', ...(named ? ['teamName', 'rosterId', 'eligibility'] : [])]); text(entry.teamId, 36); if (!uuid.test(entry.teamId)) throw Error('Invalid team id'); integer(entry.documentVersion); if (entry.documentVersion < 1) throw Error('Invalid document version'); text(entry.catalogVersion, 100); if (named) { if (typeof entry.teamName !== 'string' || typeof entry.rosterId !== 'string') throw Error('Invalid team label'); text(entry.eligibility, 80); } }
