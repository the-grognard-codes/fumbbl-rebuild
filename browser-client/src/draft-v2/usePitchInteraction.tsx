...
export function usePitchInteraction() {
  ...
  const inspected = players.find(p => p.number === inspectedId)!;
  ...
  const markers = (scene: { x: number; y: number }, scale: number) => players.map(player => <div key={player.number} className={`pitch-player ${player.team} ${player.number === activeId ? 'selected' : ''} ${player.zone === 'Prone' ? 'prone' : ''}`} data-player={player.number} style={{ left: (scene.x + player.x * 36) * scale, top: (scene.y + player.y * 36) * scale, width: 36 * scale, height: 36 * scale, zIndex: 10 + player.y * 26 + player.x }}>
    <PlayerMarker player={player} team={player.team} onSelect={() => select(player.number)} onActions={() => { if (player.team === 'home') select(player.number, false); setMenu(true); }}/><span className="pitch-number">{player.number}{player.number === ballCarrier ? ' ●' : ''}</span>
  </div>);
  const panel = <section className="pitch-command" aria-label="Player action strip">
    <div className="command-heading"><strong>{hasSelection ? `#${active.number} ${active.name} · ${active.position}` : 'No player selected'}</strong><span>{hasSelection ? `${action} · Movement ${Math.max(0, Number(active.stats[0]) - active.used)} · ${active.skills}` : 'Click or tap a player to select; repeat to deselect.'}</span><button disabled={!hasSelection} onClick={() => { setDetails(!details); }}>Details</button><button onClick={() => { setCrowded(false); setPlayers(initialPlayers); setSpent([]); setBallCarrier(1); setActiveId(1); setAction('Move'); cancel(); setMessage('Sample reset.'); }}>Reset demo</button></div>
    ...
    <div className="command-preview"><span>{target ? `${effective} → (${target.x}, ${target.y}) · ${cost ? `${cost} squares · ${risk}` : 'Review target'}${legal ? '' : ' · unavailable in this sample'}` : hover && route ? `${cost} squares · ${risk} · click to preview` : message}</span><button className="commit-action" disabled={!legal} onClick={commit}>Commit {effective.toLowerCase()}{effective === 'Move' && cost ? ` · ${cost} squares` : ''}</button></div>
    <div className="risk-legend"><span>◆ No roll</span><span>◆ Dodge</span><span>◆ Rush</span><span>◆ Dodge + rush</span><small>Illustrative shortest routes; not engine odds. Preview → commit.</small></div>
  ...
  const toggleFormation = () => {
    ...
    setPlayers(next ? Array.from({ length: 16 }, (_, i) => {
      const original = initialPlayers[i % initialPlayers.length];
  ...
  return { crowded, toggleFormation, overlay, markers, panel, cancel, setHover: (p: Square | null) => { if (hasSelection) setHover(p); }, setTarget: (p: Square | null) => { if (hasSelection) setTarget(p); } };
}
