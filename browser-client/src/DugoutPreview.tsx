...
export function PlayerMarker({ player, team, onSelect, onActions }: { player: typeof samples[number] & { sprite?: string }; team: string; onSelect?: () => void; onActions?: () => void }) {
  ...
  return <>
    <button ref={trigger} className={`dugout-player ${team}`} aria-label={`${player.name}, ${player.position}, ${player.zone}`} aria-describedby={anchor ? id : undefined}
      ...
      <img src={spriteUrl(player.sprite ?? ({ Blitzer: '02-blitzer-man.png', Catcher: '04-catcher-woman.png', Lineman: '08-lineman-man.png', Thrower: '06-thrower-man.png' }[player.position] ?? '08-lineman-man.png'), team)} alt="" className={(player.position === 'Ogre' || player.position === 'Troll') ? 'ogre-sprite' : ''}/>
...
export function Dugout({ team }: { team: string }) {
  return <div className={`dugout ${team}`} role="group" aria-label={`${team} dugout`}>
    {['Reserves', 'Knocked out', 'Casualties'].map(zone => <div className="dugout-zone" key={zone}><span>{zone} <b>{samples.filter(p => p.zone === zone).length}</b></span><div className="dugout-players">{samples.filter(p => p.zone === zone).map(player => <PlayerMarker key={player.number} team={team} player={team === 'home' ? player : { ...player, stats: player.position === 'Catcher' ? ['5', '4', '4+', '6+', '10+'] : player.stats, skills: player.position === 'Catcher' ? 'None' : player.skills, position: player.position === 'Catcher' ? 'Big Un' : player.position === 'Lineman' ? 'Line Orc' : player.position, sprite: ['02-blitzer-man.png', '05-big-un-woman.png', '08-line-orc-man.png', '06-thrower-man.png'][player.number - 7], name: ['Ash Copper', 'Fern Fleet', 'Rowan Stone', 'Vale Reed'][player.number - 7], number: player.number + 4 }}/>)}</div></div>)}
  </div>;
}
