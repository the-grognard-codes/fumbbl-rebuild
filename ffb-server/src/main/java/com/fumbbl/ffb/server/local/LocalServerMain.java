...
public class LocalServerMain {
	...
	static void validateProfile(Properties properties) {
		...
		if (marker6 && (properties.getProperty("local.browser.v2.firebase.project") == null
			|| properties.getProperty("local.browser.v2.firebase.project").trim().isEmpty())) {
			throw new IllegalArgumentException("Marker-6 local startup requires a Firebase project");
		}
